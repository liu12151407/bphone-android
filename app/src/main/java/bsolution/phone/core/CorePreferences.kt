...
class CorePreferences constructor(private val context: Context) {
    ...
    var debugLogs: Boolean
        get() = config.getBool("app", "debug", bsolution.phone.BuildConfig.DEBUG)
        set(value) {
    ...
    // mrkerusiwashere: setting to keep service alive
    ...
    // mrkerusiwashere: enable animation try
    ...
    /* Feature related */
    // mrkerusiwashere: enable screenshot
    val showScreenshotButton: Boolean
        get() = config.getBool("app", "show_take_screenshot_button_in_call", true)
    ...
    // mrkerusiwashere: disable chat Feature for SIP Server with no Chat Support
    ...
    /* Assistant */
    // mrkerusiwashere: only show assistant_generic_login
    ...
    // mrkerusiwashere: hide chat setting for SIP Server that doesn't use chat
    val showChatSettings: Boolean
        get() = config.getBool("app", "settings_chat", false)
    ...
    /* Assets stuff */
    // mrkerusiwashere: NOTE: this is where the config file reference to asset files
    ...
    private fun copy(from: String, to: String, overrideIfExists: Boolean = false) {
        ...
        if (outFile.exists()) {
            if (!overrideIfExists) {
                android.util.Log.i(context.getString(bsolution.phone.R.string.app_name), "[Preferences] File $to already exists")
                return
            }
        }
        android.util.Log.i(context.getString(bsolution.phone.R.string.app_name), "[Preferences] Overriding $to by $from asset")
    ...
    private fun move(from: String, to: String, overrideIfExists: Boolean = false) {
        ...
        if (inFile.exists()) {
            if (outFile.exists() && !overrideIfExists) {
                android.util.Log.w(context.getString(bsolution.phone.R.string.app_name), "[Preferences] Can't move [$from] to [$to], destination file already exists")
            } else {
                ...
                inFile.delete()
                android.util.Log.i(context.getString(bsolution.phone.R.string.app_name), "[Preferences] Successfully moved [$from] to [$to]")
            }
        } else {
            android.util.Log.w(context.getString(bsolution.phone.R.string.app_name), "[Preferences] Can't move [$from] to [$to], source file doesn't exists")
        }
    }
}
